...
from sklearn.linear_model import LogisticRegression
grid_model = GridSearchCV(LogisticRegression(solver='liblinear'), grid, scoring='f1', cv=5)# 寻找F-1分数最高的参数
...
## 决策树
* 简介
根据人的的自然会使用的决策过程，建立思维决策树。
决策树使用基尼不纯净度（Gini Impurity）或熵、信息增益（Entropy Information Gain）作为衡量分类质量的标准。一般步骤如下：  
    1. 确定一个能使分类质量（Quality of a split）最优的特征，即根节点；
    2. 基于数据的一个或多个特征，由根节点分离的两类中的每一类都会被分到下一个最重要特征中，以此类推，直到确定数据中各子集足够相似，或者确定以下：
        (1)相应节点上的几乎所有观测值都具有相同的类别（即纯净度）。
        (2)数据中没有其他要分类的特征。
        (3)决策树已达到先验确定的大小限制。
* sklearn库
```
